...
class ReduceNightTimeElectricEquipmentLoadsTest < Minitest::Test
  def test_reduce_night_time_electric_equipment_loads_a
    # create an instance of the measure
    measure = ReduceNightTimeElectricEquipmentLoads.new
    ...
    assert_equal('elec_load_def', arguments[0].name)

    # set argument values to good values and run the measure on model with spaces
    argument_map = OpenStudio::Measure.convertOSArgumentVectorToMap(arguments)

    elec_load_def = arguments[0].clone
    assert(elec_load_def.setValue('ASHRAE_189.1-2009_ClimateZone 1-3_LargeHotel_Lobby_ElecDef'))
    argument_map['elec_load_def'] = elec_load_def
    ...
    assert(end_sunday.setValue(0.16))
  ...
  def test_reduce_night_time_electric_equipment_loads_b
    # create an instance of the measure
    measure = ReduceNightTimeElectricEquipmentLoads.new
    ...
    elec_load_def = arguments[0].clone
    assert(elec_load_def.setValue('ASHRAE_189.1-2009_ClimateZone 1-3_LargeHotel_Cafe_ElecDef'))
    argument_map['elec_load_def'] = elec_load_def
    fraction_value = arguments[1].clone
    assert(fraction_value.setValue(0.1))
    argument_map['fraction_value'] = fraction_value
