...
class temp_class_0 < open_studio::Measure::model_measure
  ...
  def arguments(model)
    ...
    loop_args_hash.sort.map do |key, value|
      ...
      components.each do |component|
        ...
        if !component.to_pump_constant_speed.empty?
          show_loop = true
        end
        if !component.to_pump_variable_speed.empty?
          show_loop = true
        end
      end
    ...
    object.set_display_name('Choose a Plant or Air Loop to Alter.')
    object.set_default_value('*All Plant and Air Loops*') # if no loop is chosen this will run on all air loops
    args << object
    ...
    motor_eff.set_display_name('Set Motor Efficiency(%).')
    motor_eff.set_default_value(96.0)
    args << motor_eff

    # bool argument to remove existing costs
    remove_costs = open_studio::Measure::os_argument.make_bool_argument('remove_costs', true)
    remove_costs.set_display_name('Remove Baseline Costs From Effected Fans and Pumps?')
  ...
  def run(model, runner, user_arguments)
    ...
    if (motor_eff <= 0) || (motor_eff >= 100)
      runner.register_error('Please enter a number between 0 and 100 for motor efficiency percentage.')
      return false
    end
    ...
    get_total_costs_for_objects = lambda do |objects|
                runner.register_info("Changing the motor efficiency from #{initial_motor_efficiency * 100}% to #{motor_eff}% for '#{h_vac_component.name}' onloop '#{loop.name}.'")
                initial_motor_efficiency_values << initial_motor_efficiency
                h_vac_component.set_motor_efficiency(motor_eff * 0.01)
    ...
    if initial_motor_efficiency_values.size + missing_initial_motor_efficiency == 0
      runner.register_as_not_applicable('The affected loop(s) does not contain any fans or pumps, the model will not be altered.')
      return true
    end

    # reporting initial condition of model
    runner.register_initial_condition("The starting motor efficiency values in affected loop(s) range from #{initial_motor_efficiency_values.min * 100}% to #{initial_motor_efficiency_values.max * 100}%. Initial year 0 capital costs for affected fans or pumps is $#{neat_numbers(yr0_capital_total_costs_baseline, 0)}.")

    # reporting final condition of model
    runner.register_final_condition("#{initial_motor_efficiency_values.size + missing_initial_motor_efficiency} fans or pumps had motor efficiency values set to #{motor_eff}%. Final year 0 capital costs for affected fans and pumps is $#{neat_numbers(yr0_capital_total_costs_proposed, 0)}.")
...
Improve_motor_efficiency.new.register_with_application
