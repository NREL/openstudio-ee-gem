...
class IncreaseInsulationRValueForRoofs < OpenStudio::Measure::ModelMeasure
  ...
  def run(model, runner, user_arguments)
      ...
      if max_thermal_resistance <= unit_helper(min_expected_r_value_ip, 'ft^2*h*R/Btu', 'm^2*K/W')
        ...
        if one_time_retrofit_cost_ip > 0
          ...
          runner.registerInfo("Adding one time cost of #{neat_numbers(unit_helper(lcc_retrofit_specific.cost, '1/m^2',
                                                                                  '1/ft^2'))} ($/ft^2) related to retrofit of roof insulation.")
        end
    ...
    runner.registerFinalCondition("The existing insulation for roofs was changed to R-#{r_value}. This was accomplished for an initial cost of #{one_time_retrofit_cost_ip} ($/sf) and an increase of #{material_cost_increase_ip} ($/sf) for construction. This was applied to #{neat_numbers(
      affected_area_ip, 0
