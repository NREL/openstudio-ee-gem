...
def generate_url(material_name, endpoint ="materials", page_number=1, page_size=250, jurisdiction="021", date=None, option=None, boolean="yes",
    ...
    '''
    Generate a URL for fetching EPD data based on Masterformat CSI division
    jurisdiction = "021" means Northern America region
    '''
    if date is None:
...
def parse_product_epd(epd: Dict[str, Any]) -> Dict[str, Any]:
    ...
    original_ec3_link = epd.get('manufacturer', {}).get('original_ec3_link', None)
    if original_ec3_link is None:
        original_ec3_link = epd.get('short_link')

    # Per mass
    if gwp_per_kg != None:
        gwp_per_kg = extract_numeric_value(gwp_per_kg)

    elif gwp_per_kg == None and declared_unit and "t" in declared_unit:
        gwp_per_kg = divide(gwp_per_declared_unit, declared_unit)
        gwp_per_kg = gwp_per_kg/1000 # convert from t to kg

    elif gwp_per_kg == None and declared_unit and "kg" in declared_unit:
        gwp_per_kg = divide(gwp_per_declared_unit, declared_unit)
    ...
    elif declared_unit and "m3" in declared_unit and thickness and "mm" in thickness:
        gwp_per_m3 = divide(gwp_per_declared_unit, declared_unit)
...
def main():
    ...
    # print("Using Masterformat CSI divisons:")
