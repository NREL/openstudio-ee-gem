...
class temp_class_0 < open_studio::Measure::model_measure
  ...
  def run(model, runner, user_arguments)
    ...
    if (r_value < 0) || (r_value > 500)
      runner.register_error("The requested roof insulation R-value of #{r_value} ft^2*h*R/Btu was above the measure limit.")
      return false
    elsif r_value > 60
      runner.register_warning("The requested roof insulation R-value of #{r_value} ft^2*h*R/Btu is abnormally high.")
    elsif r_value < min_expected_r_value_ip
      runner.register_warning("The requested roof insulation R-value of #{r_value} ft^2*h*R/Btu is abnormally low.")
    end
    ...
    runner.register_initial_condition("The building had #{initial_string.size} roof constructions: #{initial_string.sort.join(', ')}.")
    ...
    exterior_surface_constructions.each do |exterior_surface_construction|
      ...
      if max_thermal_resistance <= unit_helper(min_expected_r_value_ip, 'ft^2*h*R/Btu', 'm^2*K/W')
        ...
        final_construction.set_name("#{exterior_surface_construction.name} adj roof insulation")
        ...
        if one_time_retrofit_cost_ip > 0
          ...
          runner.register_info("Adding one time cost of #{neat_numbers(unit_helper(lcc_retrofit_specific.cost, '1/m^2', '1/ft^2'))} ($/ft^2) related to retrofit of roof insulation.")
        end
    ...
    default_construction_sets.each do |default_construction_set|
      if default_construction_set.direct_use_count > 0
        ...
        if !default_surface_const_set.empty?
          starting_construction = default_surface_const_set.get.roof_ceiling_construction
          ...
          new_default_construction_set.set_name("#{default_construction_set.name} adj roof insulation")
          ...
          new_default_surface_const_set.set_name("#{default_surface_const_set.get.name} adj roof insulation")
          ...
          target_const = new_default_surface_const_set.roof_ceiling_construction
          if !target_const.empty?
            ...
            constructions_hash_old_new.each do |orig, new|
              if target_const == orig
                ...
                new_default_surface_const_set.set_roof_ceiling_construction(final_construction)
    ...
    if affected_area_si == 0
      runner.register_as_not_applicable('No roofs were altered.')
      return true
...
Increase_insulation_r_value_for_roofs.new.register_with_application
