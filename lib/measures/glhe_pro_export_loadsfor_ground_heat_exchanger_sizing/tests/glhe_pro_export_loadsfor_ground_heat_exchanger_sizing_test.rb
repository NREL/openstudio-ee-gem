...
class GLHEProExportLoadsforGroundHeatExchangerSizingTest < Minitest::Test
  def is_openstudio_2?
    begin
  ...
  def test_glhe_pro_export_loadsfor_ground_heat_exchanger_sizing_air_loop_only
    test_name = 'test_example_model'
    model_in_path = "#{File.dirname(__FILE__)}/ExampleModel.osm"

    # create an instance of the measure
    measure = GLHEProExportLoadsforGroundHeatExchangerSizing.new
  ...
  def test_glhe_pro_export_loadsfor_ground_heat_exchanger_sizing_good_model
    ...
    # mimic the process of running this measure in OS App or PAT
    epw_path = OpenStudio::Path.new("#{File.dirname(__FILE__)}/USA_IL_Chicago-OHare.Intl.AP.725300_TMY3.epw").to_s
    ...
    begin
      ...
      show_output(result)
      assert_equal('Success', result.value.valueName)
      assert(result.warnings.empty?) # don't want ot see No hourly heating data found for 'OS:DistrictHeating 1' on 'Heat Pump Loop'
    ensure
      Dir.chdir(start_dir)
    end
    ...
    # make sure additional reports were made
    assert(File.exist?("#{run_dir(test_name)}/Annual Hourly Loads for Heat Pump Loop.csv"))
    assert(File.exist?("#{run_dir(test_name)}/Monthly Loads for Heat Pump Loop.gt1"))
  end
end
